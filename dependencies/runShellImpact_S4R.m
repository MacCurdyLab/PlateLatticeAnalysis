function [simData] = runShellImpact_S4R(simStruct)
...
volBox = prod([plateDim boxThick]);
rhoBox = 1e-3*simStruct.mImpact/volBox;
...
fprintf(fileID,'1, \t%i,\n',size(Fplate,1));
fprintf(fileID,'*Nset, nset=nAll, generate\n');
fprintf(fileID,'1, \t%i,\n',size(Vbox,1));
fprintf(fileID,'*Solid Section, elset=allEl, material=LE1\n');
fprintf(fileID,',\n');
...
fprintf(fileID,'*Nset, nset=mBot\n');
fprintf(fileID,' 1,\n');
fprintf(fileID,' *Node\n');
fprintf(fileID,'      2,           0.,           0.,           %2.2f\n',L(6)*2);
fprintf(fileID,'*Nset, nset=mTop\n');
fprintf(fileID,' 2,\n');
...
fprintf(fileID,'*Surface, type=NODE, name=TOPPLATE-1_nAll_CNS_, internal\n');
fprintf(fileID,'TOPPLATE-1.nAll, 1.\n');
fprintf(fileID,'*Coupling, constraint name=Constraint-1, ref node=mTop, surface=TOPPLATE-1_nAll_CNS_\n');
fprintf(fileID,'*Kinematic\n');
fprintf(fileID,'*Surface, type=NODE, name=BOTTOMPLATE-1_nAll_CNS_, internal\n');
fprintf(fileID,'BOTTOMPLATE-1.nAll, 1.\n');
fprintf(fileID,'*Coupling, constraint name=Constraint-1, ref node=mBot, surface=BOTTOMPLATE-1_nAll_CNS_\n');
fprintf(fileID,'*Kinematic\n');
...
fprintf(fileID,'*Material, name=LE1\n*Density\n%2.2e,\n',rhoBox);
...
fprintf(fileID,'*Initial Conditions, type=VELOCITY\n');
fprintf(fileID,'TOPPLATE-1.nAll, 1, 0.\n');
fprintf(fileID,'TOPPLATE-1.nAll, 2, 0.\n');
fprintf(fileID,'TOPPLATE-1.nAll, 3, -%1.2f\n',simStruct.vImpact);
...
fprintf(fileID,'mBot, ENCASTRE\n');

fprintf(fileID,'*Boundary\n');
fprintf(fileID,'mTop, 1, 1\n');
fprintf(fileID,'mTop, 2, 2\n'); 
fprintf(fileID,'mTop, 4, 4\n');
fprintf(fileID,'mTop, 5, 5\n');
fprintf(fileID,'mTop, 6, 6\n');

fprintf(fileID,'*Restart, write, number interval=1, time marks=NO\n');
fprintf(fileID,'*FILE OUTPUT,NUMBER INTERVAL=');
fprintf(fileID,'%i\n',simStruct.nWriteFil);

fprintf(fileID,'*NODE FILE, NSET=mTop\nU\n');
fprintf(fileID,'*NODE FILE, NSET=mBot\nRF\n');
